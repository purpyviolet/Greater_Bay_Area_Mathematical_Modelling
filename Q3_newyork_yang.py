...
# 提取 GDP 和相关指标
gdp = data['纽约GDP (万亿美元)']
gdp_data = data[['纽约GDP (万亿美元)']]
population_data = data[['纽约人口 (百万)', '纽约年龄 0-14 岁 (百万)',
                        '纽约年龄 15-64 岁 (百万)', '纽约年龄 65 岁及以上 (百万)']]
technology_data = data[['纽约研发经费', '纽约专利申请数量']]
logistics_data = data[['纽约物流总量 (亿吨)', '纽约交通运输网络长度 (公里)']]
...
# 选择相关因素并构建自变量
X = pd.concat([population_data, technology_data, logistics_data, international_data, education_data, industry_data],
              axis=1)
Y = gdp.values
...
# Gradient Boosting模型
gb_model = GradientBoostingRegressor(n_estimators=100, learning_rate=0.1, max_depth=5, random_state=42).fit(X_train,
                                                                                                            Y_train)
...
# 可视化结果
labels1 = ['Linear Regression', 'Random Forest', 'BP Neural Network', 'Ridge Regression', 'Lasso Regression', 'XGBoost',
           'CatBoost', 'AdaBoost', 'Gradient Boosting', 'Decision Tree', 'PSO Weighted']
...
mape_scores = [MAPE_linear, MAPE_rf, MAPE_nn, MAPE_ridge, MAPE_lasso, MAPE_xgb, MAPE_catboost, MAPE_ada, MAPE_gb,
               MAPE_dt, MAPE_weighted]
rmse_scores = [RMSE_linear, RMSE_rf, RMSE_nn, RMSE_ridge, RMSE_lasso, RMSE_xgb, RMSE_catboost, RMSE_ada, RMSE_gb,
               RMSE_dt, RMSE_weighted]
...
plt.title('纽约不同模型与真实值的比较', fontsize=20)

# 显示图例
plt.legend()
plt.grid()
plt.savefig('Q3/ny_model_compare.png', dpi=300, bbox_inches='tight')
plt.show()

# R^2 Comparison
plt.figure(figsize=(8, 6))  # 设置宽度为10，高度为6
plt.bar(labels1, r2_scores, color='b')
plt.title('纽约各模型 R^2 对比', fontsize=20)
plt.ylabel('R^2', fontsize=16)
plt.xticks(rotation=90)  # 使横坐标标签纵向显示
plt.tight_layout()  # 自动调整子图参数，使图表适合
plt.savefig('Q3/ny_R2_compare.png', dpi=300, bbox_inches='tight')
plt.show()

# MAPE Comparison
plt.figure(figsize=(8, 6))
plt.bar(labels1, mape_scores, color='g')
plt.title('纽约各模型 MAPE 对比', fontsize=20)
plt.ylabel('MAPE (%)', fontsize=16)
plt.xticks(rotation=90)
plt.tight_layout()
plt.savefig('Q3/ny_MAPE_compare.png', dpi=300, bbox_inches='tight')
plt.show()

# RMSE Comparison
plt.figure(figsize=(8, 6))
plt.bar(labels1, rmse_scores, color='r')
plt.title('纽约各模型RMSE对比', fontsize=20)
plt.ylabel('RMSE', fontsize=16)
plt.xticks(rotation=90)
plt.tight_layout()
plt.savefig('Q3/ny_RMSE_compare.png', dpi=300, bbox_inches='tight')
plt.show()
...
def generate_future_data(data, years=10, growth_type='linear', model_type='linear_regression', random_state=42):
    ...
    # 拆分特征和目标值
    X = np.array(range(len(data))).reshape(-1, 1)  # 使用索引作为特征
    future_X = np.array(range(len(data), len(data) + years)).reshape(-1, 1)  # 未来的数据索引
    ...
    elif model_type == 'exponential_regression':
        model = None  # 需要单独处理
    elif model_type == 'arima':
        model = None  # ARIMA单独处理
    else:
    ...
    for col in data.columns:
        ...
        if growth_type == 'linear':
        ...
        elif growth_type == 'exponential_regression':
            # 指数回归：ln(y) = a + b * x
            # 对y进行对数转换：ln(y)
            y_log = np.log(data[col].values)
            ...
            # 预测未来的指数增长
            future_y_log = model.predict(future_X)
            new_values = a * np.exp(b * future_X.flatten())
        elif model_type == 'arima':
            # 使用 ARIMA 进行预测
            # 使用 ARIMA 模型拟合并进行预测
            model = ARIMA(data[col], order=(5, 1, 0))  # ARIMA(p, d, q), 这里选择 (5, 1, 0) 为示例
            model_fit = model.fit()
            forecast = model_fit.forecast(steps=years)
            new_values = forecast
        else:
            # 对其他模型，直接进行训练和预测
            model.fit(X, data[col].values)
            new_values = model.predict(future_X)
...
future_gdp = generate_future_data(gdp_data, years=10, growth_type='arima')
...
print(len(future_weighted))
print(len(future_gdp))
print(future_gdp)
print(future_weighted)
...
# 显示图表
plt.tight_layout()
plt.savefig('Q3/NewYork_future_predict.png', dpi=300, bbox_inches='tight')
plt.show()
...
# 粤港澳大湾区GDP数据（万亿元人民币），缺失值用 NaN 表示
...
plt.plot(year[nan_indices_gdp], gdp_interp[nan_indices_gdp], 'bx', markersize=8, label='插值数据')
plt.grid(True)
plt.title('粤港澳大湾区GDP 插值前后对比', fontsize=16)
plt.xlabel('年份')
...
plt.plot(year[nan_indices_tokyo_agri], tokyo_agri_interp[nan_indices_tokyo_agri], 'bx', markersize=8, label='插值数据')
plt.grid(True)
plt.title('东京第一产业产值 插值前后对比', fontsize=16)
plt.xlabel('年份')
plt.ylabel('第一产业产值 (万亿日元)')
plt.legend()

plt.tight_layout()
plt.savefig('Q0/data_preprocess.png', dpi=300, bbox_inches='tight')

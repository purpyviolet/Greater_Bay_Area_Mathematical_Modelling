...
# 2. 提取相关指标
population_data = data[['东京人口 (百万)',
                         '东京年龄 0-14 岁 (百万)',
                         '东京年龄 15-64 岁 (百万)',
                         '东京年龄 65 岁及以上 (百万)']].values

technology_data = data[['东京研发经费',
                         '东京专利申请数量']].values

logistics_data = data[['东京物流总量 (亿吨)',
                       '东京交通运输网络长度 (公里)']].values
...
foundation_data = data[['东京基础设施投资 (万亿日元)']].values
...
for p in ax.patches:
    ...
    ax.annotate(f'{height:.2f}',
                ...
                ha='center',
                va='bottom' if height >= 0 else 'top',  # 正值在上方，负值在下方
                fontsize=8)
...
for name, preds in results.items():
    plt.figure(figsize=(12, 6))
    plt.plot(years, Y_test, 'r--', label='真实值')  # 用年份作为 x 轴
    plt.plot(years, preds, label=name)  # 绘制模型预测值
    plt.title(f'{name} 预测结果对比')
    plt.xlabel('年份')
    plt.ylabel('预测值')
    plt.legend()
    plt.grid()
    ...
    # # 打印预测结果与真实值
    # for year, real, pred in zip(years, Y_test, preds):
    #     print(f'{name} 预测 {year} 年 GDP: 真实值 = {real:.2f}, 预测值 = {pred:.2f}')
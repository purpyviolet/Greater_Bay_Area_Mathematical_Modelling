...
population_data = data[['纽约人口 (百万)', '纽约年龄 0-14 岁 (百万)',
                        '纽约年龄 15-64 岁 (百万)', '纽约年龄 65 岁及以上 (百万)']].values
technology_data = data[['纽约研发经费', '纽约专利申请数量']].values
logistics_data = data[['纽约物流总量 (亿吨)', '纽约交通运输网络长度 (公里)']].values
...
plt.savefig('Q3_1_NY/international_pca.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.savefig('Q3_1_NY/population_tsne.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.savefig('Q3_1_NY/predict_real_gdp.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.savefig('Q3_1_NY/processed_factor_heatmap.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.title('纽约回归系数可视化', fontsize=20, fontweight='bold')

# 去掉网格
plt.grid(False)
plt.savefig('Q3_1_NY/processed_factor_coefficience.png', dpi=300, bbox_inches='tight')
plt.show()
